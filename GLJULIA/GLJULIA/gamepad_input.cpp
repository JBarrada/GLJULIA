...
void GamePadXbox::update() {
	...
	if (_controllerState.Gamepad.bRightTrigger && _controllerState.Gamepad.bRightTrigger > XINPUT_GAMEPAD_TRIGGER_THRESHOLD) {
		State.rt = _controllerState.Gamepad.bRightTrigger / 255.0;
	}
	...
	if (lstick_x) {
		State.lstick_x = lstick_x / 32768.0;
	}
	lstick_y = _controllerState.Gamepad.sThumbLY;
	if (lstick_y) {
		State.lstick_y = lstick_y / 32768.0;
	}
	...
	if (rstick_x) {
		State.rstick_x = rstick_x / 32768.0;
	}
	rstick_y = _controllerState.Gamepad.sThumbRY;
	if (rstick_y) {
		State.rstick_y = rstick_y / 32768.0;
	}
}